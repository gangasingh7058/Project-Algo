...
const Problem_Solve_Layout =({ problemId }) => {
    ...
    useEffect(()=>{
        ...
        const getproblem=async ()=>{
                        try {
                    const response=await axios.get(`http://localhost:3001/api/problem/${problemId}`);
    ...
    if(problem==null){
        ...
        return <div className="p-6 bg-black/40 backdrop-blur-sm border-2 border-cyan-400/50 rounded-lg">
      ...
      {/* testcases */}
            ...
            {problem.testCases.map((testcase, index) => (
                ...
                <div className="text-sm font-mono text-purple-300 mb-2">
                    <span className="text-cyan-300 font-bold">Test Case {index + 1}</span>
                </div>
                <div className="text-sm font-mono text-gray-300">
                    <div>
                    <span className="text-purple-400 font-bold">Input:</span>{" "}
                    <pre className="inline whitespace-pre-wrap">{testcase.input}</pre>
                    </div>
                    <div>
                    <span className="text-purple-400 font-bold">Output:</span>{" "}
                    <pre className="inline whitespace-pre-wrap">{testcase.output}</pre>
                    </div>
                </div>
                </div>
            ))}
